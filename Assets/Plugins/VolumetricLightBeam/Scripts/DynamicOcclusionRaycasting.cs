...
namespace VLB
{
    ...
    public class DynamicOcclusionRaycasting : DynamicOcclusionAbstractBase
    {
        ...
        float raycastMaxDistance { get { return m_Master.fallOffEnd * m_RangeMultiplier * m_Master.lossyScale.z; } }
        ...
        Vector3 GetDirection(uint dirInt)
        {
            ...
            switch (dirInt)
            {
                case (uint)Direction.Up:    return  m_Master.beamGlobalUp;
                case (uint)Direction.Right: return  m_Master.beamGlobalRight;
                case (uint)Direction.Down:  return -m_Master.beamGlobalUp;
                case (uint)Direction.Left:  return -m_Master.beamGlobalRight;
            }
            return Vector3.zero;
        }
        ...
        bool IsHitValid(HitResult hit)
        {
            if (hit.hasCollider)
            {
                float dot = Vector3.Dot(hit.normal, -m_Master.beamGlobalForward);
...
#endif
            var bestHit = GetBestHit(transform.position, m_Master.beamGlobalForward);
            ...
            if (IsHitValid(bestHit))
            {
                if (minSurfaceRatio > 0.5f)
                {
                    for (uint i = 0; i < GetDirectionCount(); i++)
                    {
                        ...
                        var newPt   = transform.position + dir3 * m_Master.coneRadiusEnd + m_Master.beamGlobalForward * m_Master.fallOffEnd;
            ...
            else
            {
                switch (planeAlignment)
                {
                    case PlaneAlignment.Beam:
                        SetClippingPlane(new Plane(-m_Master.beamGlobalForward, hit.point));
                        break;
        ...
        void SetPlaneWS(Plane planeWS)
        {
            ...
            {
                float dist;
                if (m_DebugPlaneLocal.Raycast(new Ray(transform.position, m_Master.beamGlobalForward), out dist))
        ...
        void OnDrawGizmos()
        {
            ...
            if (m_DebugPlaneLocal.IsValid())
            {
                var planePos = transform.position + m_DebugPlaneLocal.distance * m_Master.beamGlobalForward;
